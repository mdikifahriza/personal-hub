    ...
    return accounts
      ...
      .map((acc) => {
        const balance = transactions
          ...
          .reduce((bal, t) => {
            return t.type === 'income' ? bal + Number(t.amount) : bal - Number(t.amount);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-lg p-4">
          ...
          <p className="text-xl font-bold break-words">Rp {totalBalance.toLocaleString('id-ID')}</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        ...
        {accountsWithBalance.length === 0 ? (
          <p className="text-gray-500 text-sm">Belum ada akun. Tambahkan di tab Akun.</p>
        ) : (
          <div className="space-y-2">
            {accountsWithBalance.map((acc) => (
              <div
                key={acc.id}
                className="flex justify-between items-center gap-2 py-2 px-3 bg-gray-50 rounded"
              >
                <div className="min-w-0">
                  <p className="font-medium text-gray-900 truncate">{acc.name}</p>
                  <p className="text-xs text-gray-500 capitalize">{accountTypeLabels[acc.type]}</p>
                </div>
        ...
        {recentTransactions.length === 0 ? (
          ...
          <div className="space-y-2">
            {recentTransactions.map((trans) => (
              <div
                key={trans.id}
                className="flex items-center justify-between gap-3 py-2 px-3 bg-gray-50 rounded"
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-medium text-sm text-gray-900 truncate">
                      {getCategoryIcon(trans.category_id)} {getCategoryName(trans.category_id)}
                    </p>
          ...
          <div className="space-y-2">
            {activeRecurring.map((rec) => (
              ...
              >
                <div className="min-w-0">
                  <p className="font-medium text-sm text-gray-900 truncate">
                    {getCategoryIcon(rec.category_id)} {getCategoryName(rec.category_id)}
                  </p>
